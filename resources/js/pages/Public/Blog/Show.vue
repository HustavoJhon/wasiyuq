...
function renderContent(content: string): string {
    return content
        ...
        .map((block) => {
            if (block.startsWith('## ')) {
                return (
                    '<h3 class="mt-8 text-lg font-semibold text-foreground">' +
                    block.slice(3) +
                    '</h3>'
                );
            }
...
<template>
    <div class="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <span
            class="mt-6 inline-block rounded-full bg-[#2D6A4F]/10 px-4 py-1 text-sm font-medium text-[#2D6A4F]"
            >{{ post.category }}</span
        >
        ...
        <div
            class="mt-4 flex items-center gap-4 text-sm text-muted-foreground/70"
        >
        ...
        <div
            class="mt-8 aspect-[16/9] rounded-2xl bg-gradient-to-br from-muted to-muted/80"
        />

        <div
            class="mt-10 leading-relaxed text-muted-foreground"
