...
function statusClass(s: string): string {
    const classes: Record<string, string> = {
        ...
        adopted: 'text-blue-700 bg-blue-100',
        in_process: 'text-amber-700 bg-amber-100',
        withheld: 'text-gray-600 bg-gray-100',
    };
...
<template>
    <div>
        ...
        <div class="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-3">
            ...
            <div
                v-else
                class="flex aspect-[3/4] items-center justify-center rounded-xl bg-muted lg:col-span-1"
            >
                <svg
                ...
                <div class="rounded-xl border border-border bg-card p-6">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-muted-foreground">
                                <span
                                    class="rounded-full bg-muted px-3 py-0.5 text-xs font-medium"
                                    >{{ speciesLabel(pet.species) }}</span
                                >
                                <span
                                    class="ml-2 rounded-full px-3 py-0.5 text-xs font-medium"
                                    :class="statusClass(pet.status)"
                                    >{{ statusLabel(pet.status) }}</span
                                >
                                <span
                                    class="ml-2 rounded-full bg-muted px-3 py-0.5 text-xs font-medium text-muted-foreground"
                                    >{{ sizeLabel(pet.size) }}</span
                                >
                            </p>
                        </div>
                        <a
                            :href="'/admin/mascotas/' + pet.id + '/editar'"
                            class="rounded-lg bg-[#2D6A4F] px-4 py-2 text-sm font-medium text-white hover:bg-[#245a40]"
                            >Editar</a
                        >
                    </div>
                    ...
                    <div class="mt-6 grid grid-cols-2 gap-6 text-sm">
                        ...
                        <div>
                            ...
                            <a
                                :href="'/admin/organizaciones/' + pet.team.slug"
                    ...
                    <div class="mt-6 flex gap-4">
                        <div
                            class="rounded-lg border border-border bg-muted px-4 py-3 text-center"
                        >
