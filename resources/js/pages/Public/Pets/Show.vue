    ...
    <div class="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        >
            <div class="w-full max-w-lg rounded-2xl bg-card p-6 shadow-xl">
                ...
                <p class="mt-1 text-sm text-muted-foreground">
                    {{ pet.name }} — {{ pet.team.name }}
                </p>

                <form class="mt-6 space-y-4" @submit.prevent="submit">
                    <div>
                        <label class="text-sm font-medium text-foreground"
                            >¿Por qué quieres adoptarlo?</label
                        >
                        <textarea
                            ...
                            "
                            rows="4"
                            placeholder="Cuéntanos tu motivación (mín. 150 caracteres)..."
                        ></textarea>
                        <p
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        />
                        <label for="exp" class="text-sm text-foreground"
                            >Tengo experiencia con mascotas</label
                        >
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        <input
                            id="yard"
                            v-model="form.has_yard"
                            type="checkbox"
                            class="rounded border-border text-[#2D6A4F] focus:ring-[#2D6A4F]"
                        />
                        <label for="yard" class="text-sm text-foreground"
                            >Tengo patio o jardín</label
                        >
                    </div>
                    ...
                    <div>
                        <label class="text-sm font-medium text-foreground"
                            >Composición del hogar</label
                        >
                        <textarea
                            v-model="form.family_composition"
                            class="mt-1 w-full rounded-lg border bg-background px-3 py-2 text-sm outline-none focus:border-[#2D6A4F]"
                            :class="
                                errors.family_composition
                                    ? 'border-red-500'
                                    : 'border-border'
                            "
                            rows="2"
                            placeholder="¿Con quién vives? ¿Hay niños u otras mascotas?"
                        ></textarea>
                        <p
                            v-if="errors.family_composition"
                            class="mt-1 text-xs text-red-500"
                        >
                            {{ errors.family_composition }}
                        </p>
