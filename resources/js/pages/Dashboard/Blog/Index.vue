  ...
  <div>
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-muted-foreground">Artículos publicados por {{ currentTeam?.name }}.</p>
      </div>
      <a :href="`/${currentTeam?.slug}/blog/crear`" class="rounded-lg bg-[#2D6A4F] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#246142]">Nuevo Artículo</a>
    </div>
          ...
          <tr v-for="post in posts" :key="post.id" class="hover:bg-muted">
            ...
            <td class="px-5 py-4">
              <div class="flex items-center gap-2">
                <a :href="`/${currentTeam?.slug}/blog/${post.id}`" class="text-xs text-[#2D6A4F] hover:underline">Ver</a>
                <a :href="`/${currentTeam?.slug}/blog/${post.id}/editar`" class="text-xs text-muted-foreground hover:text-[#2D6A4F]">Editar</a>
                <button @click="destroy(post.id)" class="text-xs text-red-500 hover:text-red-700">Eliminar</button>
              </div>
            </td>
          </tr>
          <tr v-if="posts.length === 0">
            <td colspan="6" class="px-5 py-8 text-center text-muted-foreground/70">No hay artículos publicados.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="meta.last_page > 1" class="mt-6 flex justify-center gap-2">
      <a v-for="page in meta.last_page" :key="page" :href="`/${currentTeam?.slug}/blog?page=${page}`" class="rounded-lg px-3 py-1.5 text-sm" :class="page === meta.current_page ? 'bg-[#2D6A4F] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'">{{ page }}</a>
    </div>
  </div>
</template>
