...
<template>
    <article class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div class="mt-12 border-t border-[#2D6A4F]/10 pt-8">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <a
                    href="/eventos"
                    class="inline-flex items-center gap-2 rounded-xl border border-[#2D6A4F]/15 bg-gradient-to-br from-[#2D6A4F]/5 to-white dark:border-[#2D6A4F]/30 dark:from-[#2D6A4F]/15 dark:to-black/40 px-5 py-3 text-sm font-semibold text-[#2D6A4F] dark:text-emerald-400 transition hover:bg-[#2D6A4F]/10 dark:hover:bg-[#2D6A4F]/30 hover:shadow-sm"
                >
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                ...
                <div class="flex items-center gap-2 text-xs text-muted-foreground/60">
                    <span class="font-medium text-foreground/70">Compartir:</span>
                    <a
                        :href="`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(currentUrl)}`"
                        target="_blank"
                        rel="noopener noreferrer"
                        class="inline-flex items-center gap-1.5 rounded-lg bg-[#1877F2]/10 px-3 py-1.5 text-xs font-semibold text-[#1877F2] transition hover:bg-[#1877F2]/20"
                    >
                        <svg class="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>
                        Facebook
                    </a>
                    <a
                        :href="`https://wa.me/?text=${encodeURIComponent(event.title + ' ' + currentUrl)}`"
                        target="_blank"
                        rel="noopener noreferrer"
                        class="inline-flex items-center gap-1.5 rounded-lg bg-[#25D366]/10 px-3 py-1.5 text-xs font-semibold text-[#25D366] transition hover:bg-[#25D366]/20"
                    >
