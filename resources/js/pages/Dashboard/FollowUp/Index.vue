...
function applyFilter(status: string): void {
  router.get(`/${props.currentTeam?.slug}/seguimientos`, status ? { status } : {})
}
          ...
          <tr v-for="fu in followUps" :key="fu.id" class="hover:bg-muted">
            <td class="px-5 py-4">
              <div class="flex items-center gap-3">
                <div v-if="fu.adoption.pet.photos && fu.adoption.pet.photos.length > 0" class="h-10 w-10 flex-shrink-0 overflow-hidden rounded-lg bg-muted">
                  <img :src="'/storage/' + fu.adoption.pet.photos[0]" :alt="fu.adoption.pet.name" class="h-full w-full object-cover" />
                </div>
                <div>
                  <p class="font-medium text-card-foreground">{{ fu.adoption.pet.name }}</p>
                  <p class="text-xs text-muted-foreground/70">{{ speciesLabel(fu.adoption.pet.species) }}</p>
                </div>
              </div>
            </td>
            <td class="px-5 py-4">
              <p class="text-card-foreground">{{ fu.adoption.adopter.name }}</p>
              <p class="text-xs text-muted-foreground/70">{{ fu.adoption.adopter.email }}</p>
            </td>
            <td class="px-5 py-4 text-muted-foreground/70">{{ fu.notes ?? '—' }}</td>
            <td class="px-5 py-4 text-muted-foreground/70">{{ formatDate(fu.scheduled_date) }}</td>
            <td class="px-5 py-4">
              <span class="rounded-full px-3 py-0.5 text-xs font-medium" :class="statusClass(fu.status)">{{ statusLabel(fu.status) }}</span>
            </td>
            <td class="px-5 py-4">
              <a :href="`/${currentTeam?.slug}/seguimientos/${fu.id}`" class="text-sm text-[#2D6A4F] hover:underline">Ver</a>
            </td>
          </tr>
          <tr v-if="followUps.length === 0">
            <td colspan="6" class="px-5 py-8 text-center text-muted-foreground/70">No hay seguimientos registrados.</td>
          </tr>
        </tbody>
      </table>
    </div>
