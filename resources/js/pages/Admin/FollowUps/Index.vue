  ...
  <div>
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-muted-foreground">Visitas de seguimiento post-adopción.</p>
        <a href="/admin/seguimiento/crear" class="rounded-lg bg-[#2D6A4F] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#246142]">Nuevo Seguimiento</a>
      </div>
    </div>
          ...
          <tr v-for="fu in followUps" :key="fu.id" class="hover:bg-muted">
            ...
            <td class="px-5 py-4">
              <div class="flex items-center gap-2">
                <a :href="'/admin/seguimiento/' + fu.id" class="text-xs text-[#2D6A4F] hover:underline">Ver</a>
                <a :href="'/admin/seguimiento/' + fu.id + '/editar'" class="text-xs text-muted-foreground hover:text-[#2D6A4F]">Editar</a>
    ...
    <div v-if="meta.last_page > 1" class="mt-6 flex justify-center gap-2">
      <a v-for="page in meta.last_page" :key="page" :href="'/admin/seguimiento?page=' + page" class="rounded-lg px-3 py-1.5 text-sm" :class="page === meta.current_page ? 'bg-[#2D6A4F] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'">{{ page }}</a>
    </div>
  </div>
</template>
