...
function destroy(id: number): void {
  if (confirm('¿Eliminar esta mascota?')) {
    router.delete(`/${currentTeam?.slug}/mascotas/${id}`)
  }
}
</script>
  ...
  <div>
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-muted-foreground">Mascotas registradas en {{ currentTeam?.name }}.</p>
        <a :href="`/${currentTeam?.slug}/mascotas/crear`" class="rounded-lg bg-[#2D6A4F] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#246142]">Nueva Mascota</a>
      </div>
    </div>
      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-border/50">
          <tr v-for="pet in pets" :key="pet.id" class="hover:bg-muted">
            ...
            <td class="px-5 py-4 text-muted-foreground">{{ pet.slug }}</td>
            <td class="px-5 py-4 text-muted-foreground/70">{{ formatDate(pet.created_at) }}</td>
            <td class="px-5 py-4">
              <div class="flex items-center gap-2">
                <a :href="`/${currentTeam?.slug}/mascotas/${pet.id}`" class="text-xs text-[#2D6A4F] hover:underline">Ver</a>
                <a :href="`/${currentTeam?.slug}/mascotas/${pet.id}/editar`" class="text-xs text-muted-foreground hover:text-[#2D6A4F]">Editar</a>
    ...
    <div v-if="meta.last_page > 1" class="mt-6 flex justify-center gap-2">
      <a v-for="page in meta.last_page" :key="page" :href="`/${currentTeam?.slug}/mascotas?page=${page}`" class="rounded-lg px-3 py-1.5 text-sm" :class="page === meta.current_page ? 'bg-[#2D6A4F] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'">{{ page }}</a>
    </div>
  </div>
</template>
