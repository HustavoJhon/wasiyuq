...
function submitReport(): void {
    ...
    router.post(
        `/mi-adopcion/seguimientos/${props.followUp.id}/reportar`,
        formData,
        {
            onFinish: () => {
        ...
        <div class="mt-4 flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-muted-foreground">
                    ...
                    {{ followUp.adoption.team.name }}
                </p>
            </div>
            <span
                class="rounded-full px-4 py-1 text-sm font-medium"
                :class="statusClass(followUp.status)"
                >{{ statusLabel(followUp.status) }}</span
            >
        </div>

        <div
            v-if="followUp.status !== 'pending'"
            class="mt-8 rounded-xl border border-border bg-card p-6 text-center"
        >
            <p class="text-muted-foreground/70">
                Este seguimiento ya fue reportado.
            </p>
        </div>
        ...
        <div v-else class="mt-8 space-y-6">
            <div class="rounded-xl border border-border bg-card p-6">
                ...
                <div class="mt-3 space-y-2">
                    ...
                    <p class="text-xs text-muted-foreground/70">
                        Programado: {{ formatDate(followUp.scheduled_date) }}
                    </p>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-border bg-card p-6">
                ...
                <div class="mt-4 space-y-4">
                    <textarea
                        v-model="notes"
                        placeholder="Cuéntanos cómo está tu mascota, cómo va la adaptación..."
                        class="w-full rounded-lg border border-border bg-background px-4 py-3 text-sm outline-none focus:border-[#2D6A4F]"
                        rows="5"
                    ></textarea>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            multiple
                            accept="image/*"
                            class="mt-2 block w-full text-sm text-muted-foreground file:mr-4 file:rounded-lg file:border-0 file:bg-[#2D6A4F] file:px-4 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-[#246142]"
