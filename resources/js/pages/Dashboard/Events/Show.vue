  ...
  <div>
    ...
    <div class="mt-4 flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        ...
        <a :href="`/${currentTeam?.slug}/eventos/${event.id}/editar`" class="rounded-lg bg-[#2D6A4F] px-4 py-1.5 text-sm text-white hover:bg-[#1B4332]">Editar</a>
      </div>
    </div>
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-3">
      <div class="lg:col-span-2 space-y-6">
        <div class="rounded-xl border border-border bg-card p-6">
          <h2 class="text-lg font-semibold text-foreground">Descripción</h2>
          <p class="mt-3 text-sm leading-relaxed text-muted-foreground">{{ event.description }}</p>
        </div>
      </div>
        ...
        <div class="rounded-xl border border-border bg-muted p-4">
          ...
          <dl class="mt-3 space-y-3 text-sm">
            <div>
              ...
              <dd class="font-medium text-card-foreground">{{ typeLabels[event.type] ?? event.type }}</dd>
            </div>
