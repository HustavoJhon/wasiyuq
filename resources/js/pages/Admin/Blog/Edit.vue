    ...
    <div class="mx-auto max-w-4xl">
        ...
        <div class="mb-8 mt-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="flex items-center gap-3">
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl border border-[#2D6A4F]/15 bg-gradient-to-b from-white to-[#2D6A4F]/4 text-xl dark:border-[#2D6A4F]/30 dark:from-[#2D6A4F]/15 dark:to-black/40">
                    {{ categoryEmoji[props.post.category ?? ''] ?? '📝' }}
                </div>
                <div>
                    <h1 class="text-2xl font-bold text-foreground">{{ props.post.title }}</h1>
                    <p class="mt-0.5 text-sm text-muted-foreground">Editá el artículo del blog.</p>
                </div>
            </div>
            <button type="button" @click="destroy(props.post.id)" class="inline-flex items-center gap-1.5 self-start rounded-lg border border-red-200 px-3 py-2 text-sm font-medium text-red-500 transition hover:bg-red-50 dark:border-red-900/50 dark:hover:bg-red-950/30">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="grid gap-2">
                    ...
                    <Select v-model="form.team_id" name="team_id">
                        <SelectTrigger class="w-full">
                            <SelectValue :placeholder="props.post.team.name" />
                        </SelectTrigger>
                        <SelectContent>
                ...
                <div class="grid gap-5">
                    <div class="grid gap-2">
                        ...
                        <div class="relative">
                            ...
                            <Input id="title" v-model="form.title" type="text" placeholder="Título del artículo" class="pl-9" :disabled="form.processing" />
                        </div>
            ...
            <div class="flex items-center gap-4 rounded-2xl border border-[#2D6A4F]/15 bg-gradient-to-b from-white to-[#2D6A4F]/4 p-5 dark:border-[#2D6A4F]/30 dark:from-[#2D6A4F]/15 dark:to-black/40">
                ...
                <a :href="'/admin/blog/' + props.post.id" class="text-sm font-medium text-muted-foreground transition hover:text-foreground">Cancelar</a>
            </div>
        </form>
    </div>
</template>
